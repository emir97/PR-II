...
struct Printer {
	...
	bool isPrintat(Dokument &d, const char *username) {
		// provjera da li je osoba koja printa napravila modifikaciju na dokumentu
	...
	bool Printaj(Dokument &d, const char *username, DatumVrijeme datum) {
		...
		if (!modifikovao) return false;
		// provjera da li se u nazivu nalaze samo slova
		...
		// Dodavanje modifikacije "PRINTANJE"
	...
	float GetProsjecanBrojModifikacijaByVrsta(vrstaAktivnosti v, int trenutnoDokumenata = 0, int ukupanBrojModifikacija = 0, int brojModifikacijaOdredjeneVrste = 0) {
		...
		int brojUspjesniModifikacija = 0;
		for (size_t i = 0; i < _dokumenti[trenutnoDokumenata]._brojModifikacija; i++)
		{
			if (_dokumenti[trenutnoDokumenata]._modifikacije[i]._aktivnost == PRINTANJE)
				printano = true;
			if (_dokumenti[trenutnoDokumenata]._modifikacije[i]._aktivnost == v)
				brojUspjesniModifikacija++;
			brojModifikacija++;
		}
		if (printano) return GetProsjecanBrojModifikacijaByVrsta(v, trenutnoDokumenata + 1, ukupanBrojModifikacija + brojModifikacija, brojModifikacijaOdredjeneVrste + brojUspjesniModifikacija);
